    ...
    switch current_state
        case 1 % waiting for Unity to signal start
            ...
            if(UnityPort.BytesAvailable)
                ...
                if strcmp(unityMessage, 'Eyelink')
                    % fetch the trial number also?
                    fprintf(UnityPort,unityMessage);
                    current_state = 2;
                ...
                elseif strcmp(unityMessage,'Exit') % quit, maybe done experiment
                    ...
                    fprintf(UnityPort,'Restart'); % order seems off, try again.
                end
                disp(unityMessage);
                fprintf(UnityPort,[unityMessage ': ' int2str(current_state)]);
            end
        case 2
